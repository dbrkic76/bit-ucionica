...
const renderData = (arr) => {
  arr.forEach((charObj, index) => {
    ...
    button.addEventListener("click", (event) => {
      ...
      if (clickedIndexes.indexOf(index) === -1) {
        // da li index NIJE u nizu clickedIndexes?
        clickedIndexes.push(index); //
      } else {
        clickedIndexes = clickedIndexes.filter((value) => {
          return value !== index;
        }); // izbaci iz niza vec postojecu vrednost
      }
